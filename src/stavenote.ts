...
export class StaveNote extends StemmableNote {
  ...
  static format(notes: StaveNote[], state: ModifierContextState): boolean {
    ...
    const noteU_M_overlap: boolean = noteU.minLine <= noteM.maxLine + 0.5 && !noteU_M_unison;
    const noteM_L_overlap: boolean = noteM.minLine <= noteL.maxLine && noteM.minLine !== noteL.maxLine;
  ...
  drawLedgerLines(): void {
    ...
    const needsLedgerBelow: boolean = highestLine >= 6 || (this.isRest() && highestLine > 5);
    const needsLedgerAbove: boolean = lowestLine <= 0 || (this.isRest() && lowestLine < 1);
    if (!needsLedgerBelow && !needsLedgerAbove) return;
